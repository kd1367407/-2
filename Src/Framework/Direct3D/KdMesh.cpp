...
bool KdMesh::Create(const std::vector<KdMeshVertex>& vertices, const std::vector<KdMeshFace>& faces, const std::vector<KdMeshSubset>& subsets, bool isSkinMesh)
{
	...
	m_indexCount = (UINT)faces.size() * 3;;
...
void KdMesh::SetToDevice_Instancing(ID3D11Buffer* pInstanceBuffer, UINT instanceStride) const
{
	//--入力アセンブラ(IA)に情報を渡す--

	auto devCon = KdDirect3D::Instance().WorkDevContext();

	//プリミティブトポロジーを設定(三角形で組み立てる)
	devCon->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);

	//頂点バッファを2つセットする->頂点データを渡す
	UINT strides[2] = { sizeof(KdMeshVertex),sizeof(instanceStride) };//各バッファの1要素辺りのサイズ
	UINT offsets[2] = { 0,0 };//各バッファのどこから読み始めるか(0が先頭)
	ID3D11Buffer* pVertexBuffers[2] = { m_vertBuf.GetBuffer(),pInstanceBuffer };//GPUに送る2つのバッファの実体へのポインタ
	devCon->IASetVertexBuffers(0, 2, pVertexBuffers, strides, offsets);//GPUの入力スロットに入れる2つのバッファ

	//インデックスバッファをセット
	devCon->IASetIndexBuffer(m_indxBuf.GetBuffer(), DXGI_FORMAT_R32_UINT, 0);
}
