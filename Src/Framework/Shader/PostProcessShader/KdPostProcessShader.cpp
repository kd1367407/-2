...
#include "KdPostProcessShader_PS_Bright.shaderInc"
	...
	// ワイプ用シェーダのコンパイルと生成
#include "../Src/Framework/Shader/PostProcessShader/Transition_PS.shaderInc"
		if (FAILED(KdDirect3D::Instance().WorkDev()->CreatePixelShader(compiledBuffer, sizeof(compiledBuffer), nullptr, &m_psWipe))) {
			assert(0 && "ワイプシェーダーの作成に失敗");
			Release();
			return false;
		}
...
void KdPostProcessShader::DrawWipe(float progress)
{
	//シェーダに進捗度を送る
	m_cbWipe.Work().progress = progress;
	m_cbWipe.Work().enable = 1;
	m_cbWipe.Write();

	//サンプラーステートを「繰り返し(Wrap)」モードに変更する
	KdShaderManager::Instance().ChangeSamplerState(KdSamplerState::Point_Wrap, 0);

	//描画パイプライン設定
	auto devCon = KdDirect3D::Instance().WorkDevContext();
	devCon->VSSetShader(m_VS, nullptr, 0);//共通の頂点シェーダ(m_VSという頂点シェーダを使ってという指示)
	devCon->IASetInputLayout(m_inputLayout);//頂点シェーダ:GPUに「これから送る頂点データは『位置』と『UV座標』という情報で構成されていますよ」と教えている
	devCon->PSSetConstantBuffers(0, 1, m_cbWipe.GetAddress());//ピクセルシェーダー:cbWipeとい定数バッファの情報を送っている
	devCon->PSSetShaderResources(0, 1, KdAssets::Instance().m_textures.GetData("Asset/Textures/System/WhiteNoise.png")->WorkSRViewAddress());//ピクセルシェーダー:テクスチャをGPUに転送
	devCon->PSSetShader(m_psWipe, nullptr, 0);//ピクセルシェーダー:GPUに「これから描画するピクセルには、この『ピクセルシェーダー』（Transition_PS.hlsl）というレシピを使ってください」と指示
