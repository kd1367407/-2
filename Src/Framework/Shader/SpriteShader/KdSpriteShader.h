...
public:
	...
	{
		m_cb0.Work().gridEnable = enable;
	}

	void SetGridUVOffset(Math::Vector2 offset1, Math::Vector2 offset2)
	{
		m_cb0.Work().uvOffset1 = offset1;
		m_cb0.Work().uvOffset2 = offset2;
	}

	void SetGridNomalTexture(KdTexture& nomalTex1,KdTexture& nomalTex2)
	{
		KdDirect3D::Instance().WorkDevContext()->PSSetShaderResources(1, 1, nomalTex1.WorkSRViewAddress());
		KdDirect3D::Instance().WorkDevContext()->PSSetShaderResources(2, 1, nomalTex2.WorkSRViewAddress());
	}
