...
//タイルワイプのノイズテクスチャ
Texture2D g_wipeNoiseTex : register(t13);
...
float4 main(VSOutput In) : SV_Target0
{
	...
	if(g_wipeEnable)
	{
		//このピクセルの画面上の座標(UV。つまり画面全体からの相対位置)を計算
		//In.svPos.xy はピクセル座標、g_screenSize.xy は画面全体の解像度(ピクセル寸法)
		float2 screenUV = In.Pos.xy / g_screenSize.xy;

		//ノイズテクスチャからこのピクセル位置に対応する値を(0~1)取得
		//Sampleは画像の色を取ってくる,g_ssはどのように読むかというオプション的な感じ
		//このg_ssはKdStandardShaderで設定されているデフォルト
		float noiseValue = g_wipeNoiseTex.Sample(g_ss, screenUV).r;

		//ノイズの値が進捗度より小さければ描画しない
		if(noiseValue<g_wipeProgress)
		{
			clip(-1);
		}
	}
