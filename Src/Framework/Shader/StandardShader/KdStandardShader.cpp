...
void KdStandardShader::SetWipeInfo(float progress, bool enable)
{
	m_cb3_Wipe.Work().progress = progress;
	m_cb3_Wipe.Work().enable = enable; 
...
void KdStandardShader::BeginLit()
{
	...
	if (KdShaderManager::Instance().SetPixelShader(m_PS_Lit))
	{
		...
		//ワイプ用定数バッファを3番スロットにセット
		KdShaderManager::Instance().SetPSConstantBuffer(3, m_cb3_Wipe.GetAddress());
	}
	...
	//ノイズテクスチャを12番スロットにセット
	KdDirect3D::Instance().WorkDevContext()->PSSetShaderResources(13, 1, KdAssets::Instance().m_textures.GetData("Asset/Textures/System/WhiteNoise.png")->WorkSRViewAddress());
}
