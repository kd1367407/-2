...
void KdShaderManager::Init()
{
	...
	//遷移エフェクト(cbFrameの生成とバインド)
	m_cb6_Frame.Create();
	KdDirect3D::Instance().WorkDevContext()->VSSetConstantBuffers(6, 1, m_cb6_Frame.GetAddress());
	KdDirect3D::Instance().WorkDevContext()->PSSetConstantBuffers(6, 1, m_cb6_Frame.GetAddress());
...
void KdShaderManager::WriteCBFrame()
{
	//作業領域に画面サイズを書き込む
	cbFrame& frame = m_cb6_Frame.Work();

	auto backBuffer = KdDirect3D::Instance().GetBackBuffer();
	if (backBuffer)
	{
		frame.screenSize.x = (float)backBuffer->GetWidth();
		frame.screenSize.y = (float)backBuffer->GetHeight();
	}
